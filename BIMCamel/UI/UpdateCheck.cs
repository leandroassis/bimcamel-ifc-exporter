...
{
    /// <summary>
    /// Once-a-day, fire-and-forget check of the newest GitHub release when the exporter pane opens.
    /// If it is newer than this build, offers (once per version) to open the download page.
    /// Every failure path — offline, proxy, rate limit, bad JSON — is silent: the exporter must
    /// never be slowed down or degraded by the check. Mirrors the Dyncamelo update check so all
    /// BIMCamel tools behave the same.
    /// </summary>
    internal static class UpdateCheck
    {
        private const string Owner = "mrshoma99-rgb";
        private const string Repo = "bimcamel-ifc-exporter";
        private const string ProductName = "BIMCamel IFC Exporter";
        private const string DownloadPage = "https://github.com/" + Owner + "/" + Repo + "/releases/latest";

        // %APPDATA%\BIMCamel\update-check.txt: "<last check yyyy-MM-dd>|<last version offered>"
        private static readonly string StatePath = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
            "BIMCamel", "update-check.txt");
        ...
        public static void Run(Action<Action> onUiThread)
        {
            ...
            Task.Run(() =>
            {
                try
                {
                    ...
                    if (current == null || (current.Major == 0 && current.Minor == 0)) return; // dev build

                    var (lastCheck, lastOffered) = ReadState();
                    if (lastCheck == DateTime.UtcNow.Date) return; // at most one request per day
                    ...
                    if (latest <= currentThree) return;
                    if (lastOffered != null && latest <= lastOffered) return; // already declined this one
        ...
        private static void Prompt(Version current, Version latest)
        {
            ...
            {
                try { Process.Start(new ProcessStartInfo(DownloadPage) { UseShellExecute = true }); }
                catch { }
            }
        }
        ...
        private static Version? FetchLatestVersion()
        {
            ...
            request.UserAgent = "BIMCamel-IFC-Exporter-UpdateCheck";
