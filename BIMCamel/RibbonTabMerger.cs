...
namespace BIMCamel
{
    ...
    ///
    /// Every BIMCamel plug-in (this exporter, Dyncamelo, …) declares the same ribbon tab id
    ...
    /// (RibbonControl → Tabs → Panels, all ObservableCollections) is stable across 2024-2027.
    ...
    internal static class RibbonTabMerger
    {
        ...
        private static void Merge()
        {
            ...
            if (tabs == null) return;
            if (tabs.Count == _lastCount) return; // nothing new composed since last look
            ...
            {
                if (tab == null || !IsBimCamelTab(tab)) continue;
                if (primary == null) primary = tab;
                else duplicates.Add(tab);
            }
            if (primary == null || duplicates.Count == 0) return;

            if (!(GetProp<object>(primary, "Panels") is IList primaryPanels)) return;
        ...
        private static IList? FindTabsViaPresentationSources()
        {
            ...
            {
                if (!(source.RootVisual is DependencyObject root)) continue;
                var ribbon = FindDescendantOfType(root, ribbonType);
                if (ribbon != null) return GetProp<object>(ribbon, "Tabs") as IList;
            }
            return null;
        }
    ...
    /// </summary>
    [Autodesk.Navisworks.Api.Plugins.Plugin("BIMCamel.Startup", "BIMCamel",
        DisplayName = "BIMCamel IFC Exporter startup")]
    public class BIMCamelStartup : Autodesk.Navisworks.Api.Plugins.EventWatcherPlugin
    {
