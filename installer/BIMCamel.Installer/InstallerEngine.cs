...
/// Same engine as the Dyncamelo installer, rebranded for the IFC exporter.
/// </summary>
public static class InstallerEngine
{
    private const string BundleFolderName = "BIMCamel.bundle";
    private const string PayloadResource = "BIMCamel.Installer.payload.zip";
    private const string UninstallKeyPath =
        @"Software\Microsoft\Windows\CurrentVersion\Uninstall\BIMCamelIfcExporter";

    // The pre-0.6 Inno Setup installer registered under this per-user key; a new-style
    // install replaces the files, so its Apps & Features entry must go too.
    private const string LegacyInnoUninstallKeyPath =
        @"Software\Microsoft\Windows\CurrentVersion\Uninstall\{8A2F1B3C-9D4E-4A5F-8B6C-7E1F2A3B4C5D}_is1";
    ...
    /// <summary>Whether a BIMCamel bundle is currently installed.</summary>
    ...
    /// <summary>This installer's own version, e.g. "0.6.0".</summary>
    ...
    public static readonly string[] SupportedYears = { "2024", "2025", "2026", "2027" };
        ...
        try
        {
            ...
            if (!File.Exists(Path.Combine(temp, BundleFolderName, "PackageContents.xml")))
            {
                throw new InvalidDataException(
                    ...
                    "Please re-download BIMCamelSetup.exe from bimcamel.com.");
            }

            progress(85, "Removing previous version…");
            EvacuateSelfFromBundle();
            DeleteBundleDir();
            RemoveLegacyInnoEntry();
    ...
    public static void Uninstall()
    {
        ...
        {
            key?.DeleteSubKeyTree("BIMCamelIfcExporter", throwOnMissingSubKey: false);
        }
    ...
    private static void RemoveLegacyInnoEntry()
    {
        try
        {
            using var key = Registry.CurrentUser.OpenSubKey(
                Path.GetDirectoryName(LegacyInnoUninstallKeyPath)!, writable: true);
            key?.DeleteSubKeyTree(Path.GetFileName(LegacyInnoUninstallKeyPath), throwOnMissingSubKey: false);
        }
        catch (Exception ex) when (ex is System.Security.SecurityException || ex is UnauthorizedAccessException)
        {
            // A stale Apps & Features row is cosmetic; never fail the install over it.
        }
    }
    ...
    private static Stream OpenPayload(out string source)
    {
        ...
        // Fallback for the zip-style distribution: a BIMCamel.bundle folder
        ...
        throw new FileNotFoundException(
            "This BIMCamelSetup.exe carries no bundle payload and no BIMCamel.bundle " +
    ...
    {
        var target = Path.Combine(BundleDir, "BIMCamelSetup.exe");
    ...
    {
        var exe = Path.Combine(BundleDir, "BIMCamelSetup.exe");
        using var key = Registry.CurrentUser.CreateSubKey(UninstallKeyPath);
        key.SetValue("DisplayName", "BIMCamel IFC Exporter for Navisworks");
        ...
        key.SetValue("URLInfoAbout", "https://www.bimcamel.com/Export-Navisworks-to-Ifc");
    ...
    private static void EvacuateSelfFromBundle()
    {
        ...
        var temp = Path.Combine(
            Path.GetTempPath(), "BIMCamelSetup-" + Guid.NewGuid().ToString("N") + ".exe");
        try
        {
